...
*    "! Returns an exception if it's downcast
*    "! @parameter from_rtti | Source type
*    "! @parameter to_rtti | Target type
*    "! @parameter result | f
*    "! @raising zcx_convertible | f
...
*      IS_downcast( from_rtti = cast #( source_rtti )
*                        to_rtti = cast #( target_rtti ) ).
...
*       AND source_rtti->type_kind <> target_rtti->type_kind.
      " Not the same type of reference
      RAISE EXCEPTION TYPE zcx_convertible
...
*      IF target_rtti_ref->type_kind <> source_rtti_ref->type_kind.
...
*    ENDIF.
*
*    IF to_rtti->type_kind = to_rtti->typekind_dref.
*      " REF TO DATA
...
*          OR cl_abap_datadescr=>typekind_iref
